...
echo "Following MS/Office365 Subscriptions have less than $tres licenses available:" >> $body
echo "" >> $body
...
$erg = Get-MsolAccountSku | where { $_.SkuPartNumber -match "OFFICESUBSCRIPTION" }
...
if ( $chk -eq 1 )
{
	...
	Send-MailMessage -SmtpServer $mrel -To $mail -From $mrep -Subject "Office365 ProPlus: Available licenses report" -body (gc $body | Out-String)
	if ( $? )
	{
